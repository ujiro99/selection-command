...
export function showReviewRequestToast(toast: any, onAccept: () => void): void {
  const tst = toast({
    ...
    description: <span className="text-sm">{t('review_request_message')}</span>,
    className: 'flex flex-col text-gray-800',
    action: (
      <div className="w-full flex flex-row gap-3 px-4 pt-3 pb-2">
        <ToastAction
          key="later"
          altText="Later"
        ...
        >
          <span className="mr-1">🎉</span> {t('review_request_button')}
        </ToastAction>
      </div>
    ),
    duration: 60 * 1000,
