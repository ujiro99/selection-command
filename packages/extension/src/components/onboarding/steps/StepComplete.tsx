...
import { MousePointerClick, PartyPopper, Settings } from "lucide-react"
...
export function StepComplete({ onboarding }: Props) {
  ...
  const delays = [600, 200, 800, 1000, 1200] // ms
    ...
    <>
      ...
      <div className="flex flex-col items-center gap-8">
        ...
        <OnboardingFadeIn
          ...
          delay={delays[1]}
        >
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-700">
        ...
        <div className="mt-2.5 flex items-center gap-5">
          ...
          >
            <button
              type="button"
              className="flex items-center gap-3 min-h-14 rounded-xl bg-sky-950 pl-6 pr-8 text-[15px] font-semibold text-white shadow-[0_10px_20px_-14px_rgba(15,23,42,.7)] hover:brightness-[1.35]"
          ...
          >
            <button
              type="button"
              className="flex items-center gap-3 min-h-14 rounded-xl px-5 text-[15px] font-medium text-slate-500 border border-slate-200 hover:bg-slate-50 hover:text-slate-700"
